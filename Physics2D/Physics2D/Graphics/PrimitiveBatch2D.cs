...
namespace Physics2D.Graphics
{
    ...
    public class PrimitiveBatch2D
    {
        ...
        public void Begin(PrimitiveType primType, Camera2D cam)
        {
            if (hasBegun)
            {
        ...
        public void DrawRotatedCircle(Circle c, float rotation, Color startColor, Color endColor)
        {
            DrawRotatedCircle(c.Center, c.Radius, rotation, startColor, endColor);
        }

        public void DrawRotatedCircle(Vector3 center, float radius, float rotation, Color startColor, Color endColor)
        {
            int iterations = 20;

            Vector3 prevPos = center + new Vector3((float)Math.Cos(rotation) * radius, (float)Math.Sin(rotation) * radius, 0);
            ...
            for (int i = 0; i <= iterations; i++)
            {
                ...
                angle += rotation;
                Vector3 pos = center + new Vector3((float)Math.Cos(angle) * radius, (float)Math.Sin(angle) * radius, 0);
                Color color = Color.Lerp(startColor, endColor, (float)i / iterations);
                AddVertex(new VertexPositionColor(prevPos, color));
                AddVertex(new VertexPositionColor(pos, color));
                prevPos = pos;
            }

            Vector3 crossVert = new Vector3((float)Math.Cos(rotation) * radius, (float)Math.Sin(rotation) * radius, 0);
            DrawLine(center + crossVert, center - crossVert, Color.Orange);
            crossVert = new Vector3((float)Math.Cos(rotation + MathHelper.PiOver2) * radius, (float)Math.Sin(rotation + MathHelper.PiOver2) * radius, 0);
            DrawLine(center + crossVert, center - crossVert, Color.Orange);
        }
        ...
        public void DrawVertexBuffer(VertexBuffer buffer, PrimitiveType primType, Camera2D cam)
        {
